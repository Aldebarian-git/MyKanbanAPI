    ...
    validate(req, method) {
        let schema = Joi.object({
            content: Joi.string().min(3).messages({
                ...
                "string.min":
                    "Le contenu doit contenir au moins {#limit} caractères",
            }),
            ...
            list_id: Joi.number().integer().greater(0).messages({
                ...
                "number.integer":
                    "L'identifiant de la liste doit être un nombre entier",
                "number.greater":
                    "L'identifiant de la liste doit être supérieur à {#limit}",
            }),
        });

        if (req.method === "POST") {
            schema = schema.fork(["content", "list_id"], field =>
