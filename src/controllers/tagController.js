...
const tagController = {
    ...
    async create(req, res, next) {
        ...
        try {
            ...
            // 409: Conflict => Le nom du tag en BDD doit être unique, hors la valeur en cours existe déjà
            return next({
                statusCode: 409,
                message: "Il existe déjà un tag avec ce nom",
            });
        }
    },
    ...
    validate(req) {
        let schema = Joi.object({
            name: Joi.string().min(3).max(50).messages({
                ...
                "string.min":
                    "Le nom doit contenir au moins {#limit} caractères",
                "string.max":
                    "Le nom doit contenir au plus {#limit} caractères",
            }),
            color: Joi.string()
